...
void runSgemm1DBlocktiling(int M, int N, int K, float alpha, float *A,
                           float *B, float beta, float *C) {
  const uint BM = 64;
  const uint BN = 64;
  const uint BK = 8;
...
void runSgemm2DBlocktiling(int M, int N, int K, float alpha, float *A,
                           float *B, float beta, float *C) {
  const uint BK = 8;
  const uint TM = 8;
  const uint TN = 8;
...
void runSgemmWarptiling(int M, int N, int K, float alpha, float *A, float *B,
  ...
  static_assert((K10_BN % K10_WN == 0) &&
                (K10_BM % K10_WM == 0));

  static_assert((K10_BN / K10_WN) *
                    (K10_BM / K10_WM) ==
                NUM_WARPS);

  static_assert((K10_WM * K10_WN) %
                    (WARPSIZE * K10_TM * K10_TN * K10_WNITER) ==
                0);

  constexpr uint K10_WMITER =
      (K10_WM * K10_WN) /
      (32 * K10_TM * K10_TN * K10_WNITER);

  static_assert((K10_WM % K10_WMITER == 0) &&
                (K10_WN % K10_WNITER == 0));
  ...
  static_assert((K10_BM * K10_BK) %
                    (4 * K10_NUM_THREADS) ==
                0);
  static_assert((K10_BN * K10_BK) %
                    (4 * K10_NUM_THREADS) ==
                0);

  dim3 gridDim(CEIL_DIV(N, K10_BN),
               CEIL_DIV(M, K10_BM));
...
void runSgemmDoubleBuffering(int M, int N, int K, float alpha, float *A,
  ...
  static_assert((K11_BN % K11_WN == 0) &&
                (K11_BM % K11_WM == 0));

  static_assert((K11_BN / K11_WN) *
                    (K11_BM / K11_WM) ==
                NUM_WARPS);

  static_assert((K11_WM * K11_WN) %
                    (WARPSIZE * K11_TM * K11_TN * K11_WNITER) ==
                0);

  constexpr uint K11_WMITER =
      (K11_WM * K11_WN) /
      (32 * K11_TM * K11_TN * K11_WNITER);

  static_assert((K11_WM % K11_WMITER == 0) &&
                (K11_WN % K11_WNITER == 0));

  static_assert((K11_NUM_THREADS / 2 * 4) %
                    K11_BK ==
                0);
  ...
  static_assert((K11_BM * K11_BK) %
                    (4 * K11_NUM_THREADS / 2) ==
                0);

  static_assert((K11_BN * K11_BK) %
                    (4 * K11_NUM_THREADS / 2) ==
                0);

  dim3 gridDim(CEIL_DIV(N, K11_BN),
               CEIL_DIV(M, K11_BM));
...
// Kernel 12 intentionally omitted on P100.
// It uses CUDA synchronization primitives requiring sm_70+.
...
void run_kernel(int kernel_num, int M, int N, int K, float alpha, float *A,
  ...
  default:
    throw std::invalid_argument(
        "Unknown kernel number. P100 build supports kernels 0-11.");
  }
}